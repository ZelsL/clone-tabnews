  ...
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/f39498d0-e9b6-4ebc-8886-3a147f9e9cc5",
        {
          method: "PATCH",
        },
      );
    ...
    test("With expired token", async () => {
      jest.useFakeTimers({
        now: new Date(Date.now() - activation.EXPIRATION_IN_MILISSECONDS),
      });
    ...
    test("With valid token", async () => {
      ...
      expect(expiresAt - createdAt).toBe(activation.EXPIRATION_IN_MILISSECONDS);
